...
class MetricsCallback(Callback):
    ...
    def after_batch(self, last_output, last_target):
        ...
        for image in range(0, last_target.shape[0]):
            ...
            self.metrics["drd_k sum"] += drd(pred[image], text_mask).sum().item()
            ...
            addIntersectArea(regionsDict, np.array(labels).copy(), pred[image], "intersectArea")
            ...
            labelsClone = np.array(labels).copy()
    ...
    def after_epoch(self):
        self.calculateMetrics()
        # Optionally, you can log or print metrics here
        ...
        for mode in self.modes:
            ...
            for dataset in self.datasets:
                ...
                for metric in ["#fn", "#fp", "#tp expanded", "#tp eroded"]:
                    ...
                    modeMetrics[dataset][metric] += metrics[metric]
    ...
    def data(self, data):
        ...
        if isinstance(data, pd.DataFrame):
            ...
            l = int(data.get_group(index).shape[0])
...
def getDatasetMetrics(dataset, learn):
    ...
    for idx in range(len(dataset.valid.x.items)):
        ...
        m.after_batch(pred, dataset.valid.y.get(idx, False))
    m.calculateMetrics()
    return m
...
def addWatershed(regionsDict, labels, mask, prop):
    mask = np.asarray(mask, dtype=bool)  # Ensure mask is a boolean NumPy array
    distance = nd.distance_transform_edt(mask)
    distance = np.asarray(distance)      # Ensure distance is a NumPy array
...
W /= W.sum()
W = torch.as_tensor(W).unsqueeze(0).unsqueeze(0).float().to(device)

def drd(im, im_gt):
    # Ensure both inputs are torch tensors
    global W
    im = torch.as_tensor(im, device=device)
    im_gt = torch.as_tensor(im_gt, device=device)
    ...
    m1 = (im == 1) & (im_gt == 0)
    m2 = (im == 0) & (im_gt == 1)
    input1 = torch.as_tensor((im_gt == 0), device=device).float()
    input2 = torch.as_tensor((im_gt == 1), device=device).float()
    W_tensor = W if isinstance(W, torch.Tensor) else torch.tensor(W, dtype=input1.dtype, device=input1.device)
    conv1 = F.conv2d(input1, W_tensor, bias=None, padding=2, stride=(1, 1))
    conv2 = F.conv2d(input2, W_tensor, bias=None, padding=2, stride=(1, 1))
    return (conv1 * m1 + conv2 * m2).sum(dim=(1,2,3))
